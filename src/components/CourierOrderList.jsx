...
export default function CourierOrderList() {
  ...
  const completeOrder = async (id) => {
    ...
    updateCourierOrderStatus(courierData);
    setOpenDetails(false);
    setAlert("visible");
    setTimeout(() => {
      setAlert("invisible");
  ...
  useEffect(() => {
    // if (selectedID) {
    setSelectedOrder(orders.find((order) => order.orderID === selectedID));
    // }
  }, [selectedID, orders]);
  ...
  const fetchData = async () => {
    await getCourierOrders().then((data) => {
      console.log(data);
      setOrder(data);
      setValue(parseInt(data[0]?.rating));
      const updatedRows = data.map((order, index) => {
        let [date, time] = order.acceptOrderDate.split(" ");
        ...
        return {
          ...
          acceptDate: new Date(formattedDate),
  ...
  return (
    <>
      <div className="flex items-center justify-between mt-4 mb-4 ">
        <h1 className="text-gray-700 text-4xl font-semibold">Courier Orders</h1>
        <Stack
        ...
        >
          <Alert severity="success" variant="filled">
            ...
            <p className="font-semibold text-xl">Order has been completed!</p>
          </Alert>
        </Stack>
      </div>
      <Box
            ...
            {selectedOrder?.orderStatus === "On the way" ? (
              <div className=" bg-emerald-500 hover:bg-orange-500 flex items-center justify-center">
                <Button
                  ...
                  onClick={() => completeOrder(selectedOrder?.orderID)}
                >
