...
const EditFoodContainer = ({ data }) => {
  const navigate = useNavigate();
  const [title, setTitle] = useState(data.title);
  const [desc, setDesc] = useState(data.desc);
  const [price, setPrice] = useState(data.price);
  const [category, setCategory] = useState(data.category);
  const [imageAsset, setImageAsset] = useState(data.imageURL);
  ...
  const [foodId, setFoodId] = useState(data.id);
  ...
  const deleteDatabaseImage = () => {
    ...
    try {
      ...
      const storageRef = ref(storage, `Food_images/${imageAsset.name}`);

      deleteObject(storageRef).then(() => {
        deleteFoodData(foodId);
        setImageAsset(null);
        setIsLoading(false);
        setMsg("food delete on database successfully!");
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const storageRef = ref(storage, `Food_images/${imageFile.name}`);

      // Check if an image with the same name already exists
      //   try {
      //     const existingImageRef = ref(storage, `Food_images/${imageFile.name}`);
    ...
    <>
      <div className="w-full min-h-screen flex items-center justify-center">
        <div className="w-[90%] md:w-[50%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
          <p className="text-gray-700 text-2xl">Edit Food</p>
          ...
          <div className="w-full flex flex-col md:flex-row items-center gap-3">
            <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
              ...
              <textarea
                ...
                cols="100"
                value={desc}
                placeholder="Input your food description"
                className="w-full h-full text-lg bg-transparent outline-none border-none placeholder:text-gray-400 text-textColor"
                ...
                {!imageAsset ? (
                  ...
                  <>
                    ...
                    <div className="relative h-full p-4">
                      <img
                        src={selectedImg ? selectedImg : imageAsset}
                        alt="uploaded image"
          ...
          <div className="flex items-center w-full">
            <button
              type="button"
              className="ml-0 md:ml-auto w-full md:w-auto border-none outline-none bg-emerald-500 hover:bg-orange-400 px-12 py-2 rounded-lg text-lg text-white font-semibold"
              // onClick={saveDetails}
              onClick={uploadImage}
            >
              Update
